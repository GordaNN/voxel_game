        ...
        int         height)
{
    ...
    glfwSetInputMode(window_p->window_p, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
    glfwSetCursorPos(window_p->window_p, 0, 0);

    glfwSetKeyCallback(window_p->window_p, window_key_callback);
    glfwSetCursorPosCallback(window_p->window_p, window_cursor_pos_callback);

    glfwMakeContextCurrent(window_p->window_p);
    return true;
}
...
void window_set_size(
        ...
        int      height)
{
    glfwSetWindowSize(window.window_p, width, height);
    const GLFWvidmode* mode_p = glfwGetVideoMode(window.monitor_p);
    glfwSetWindowPos(
            window.window_p,
...
bool window_is_closed(
        window_t window)
{
    return glfwWindowShouldClose(window.window_p);
}
...
void window_close(
        window_t window)
{
    glfwSetWindowShouldClose(window.window_p, GL_TRUE);
}
...
void window_swap_buffers(
        window_t window)
{
    glfwSwapBuffers(window.window_p);
}
